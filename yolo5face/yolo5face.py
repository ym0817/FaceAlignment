...
class Yolov5face:
    def __init__(self, yolo_type = 'yolo5face/yolov5l', confThreshold=0.5, nmsThreshold=0.5, objThreshold=0.5):
    ...
    def postprocess(self, frame, outs):
        ...
        dets = [boxes[i] for i in indices]
        landms = [landmarks[i] for i in indices]
        # for i in indices:
        #     i = i[0]
        #     box = boxes[i]
        #     left = box[0]
    ...
    def inference_one(self, img_raw):
        blob = cv2.dnn.blobFromImage(img_raw, 1 / 255.0, (self.inpWidth, self.inpHeight), [0, 0, 0], swapRB=True, crop=False)
        ...
        frameHeight = img_raw.shape[0]
        frameWidth = img_raw.shape[1]
        ratioh, ratiow = frameHeight / self.inpHeight, frameWidth / self.inpWidth
        # Scan through all the bounding boxes output from the network and keep only the
        # ones with high confidence scores. Assign the box's class label as the class with the highest score.
...
if __name__ == "__main__":
    ...
    yolonet = Yolov5face(args.yolo_type, confThreshold=args.confThreshold, nmsThreshold=args.nmsThreshold, objThreshold=args.objThreshold)
